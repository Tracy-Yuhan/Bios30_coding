...
def ext12_encode(message):
    ...
    if encoded_mes != False:
        ...
        print('By using the number-codon conversion dictionary, a numerical checksum value can be obtained, and that is: ', checksum_val)
        checksum_codon = checksum_to_codons(checksum_val)
        print('The numerical checksum value is then turned into a 4-bases codon: ', checksum_codon)
        encoded_mes = list_to_string(encoded_mes)
        encoded_mes = encoded_mes.upper()
        print('The DNA seq before adding the checksum codon: ', encoded_mes)
        after_checksum = add_checksum_codon(encoded_mes,checksum_codon)
        print('The DNA seq after adding the checksum codon: ', after_checksum)
        ...
        print('After inserting into a random DNA library: ',final_mes)
...
def ext12_decode(filename):
    ...
    print('Undertake a checksum test and check the intergrity of the encoded message')
